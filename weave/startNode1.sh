...
--link core:core -v /etc/localtime:/etc/locatime -v /etc/timezone:/etc/timezone \
-i registry.local/opscenter
echo "OpsCenter started, sleeping a while"
sleep 10
# Cassandra1
sudo weave run 10.0.1.2/24 -t -p 9160:9160 -p 1021:22 --dns 127.0.0.1 \
-h cassandra1 --link core:core -v /etc/localtime:/etc/locatime \
-v /etc/timezone:/etc/timezone -i registry.local/cassandra
echo "Cassandra1 started, sleeping a while"
sleep 10
# Cassandra2
sudo weave run 10.0.1.3/24 -t -p 1022:22 --dns 127.0.0.1 -h cassandra2 --link core:core \
-v /etc/localtime:/etc/locatime -v /etc/timezone:/etc/timezone \
...
-v /etc/localtime:/etc/locatime -v /etc/timezone:/etc/timezone \
-i registry.local/cassandra
echo "Cassandra3 started, sleeping a while"
sleep 10
# Node for testing
sudo weave run 10.0.1.20/24 -t -p 1001:22 --dns 127.0.0.1 -h node --link core:core \
-v /etc/localtime:/etc/locatime -v /etc/timezone:/etc/timezone -v /home/vagrant/Projects:/root/Projects \
